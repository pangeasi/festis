...
function getFestivalDescription(festival: Festival) {
  const parts = [
    ...
    festival.artists.length ? `Cartel: ${festival.artists.slice(0, 6).join(', ')}` : null,
  ].filter(Boolean);
...
function getMetadata(url: string, origin: string, festival: Festival | null): SeoMetadata {
  const baseUrl = getBaseUrl(origin);
  ...
  return {
    ...
    imageUrl: toAbsoluteUrl(festival.image_full_url || festival.image_url, baseUrl),
  ...
  const metadata =
    status === 404
      ? {
          ...
          description: 'La ficha de festival solicitada no existe o ha cambiado de direccion.',
...
export function getSitemapEntries(origin: string) {
  const baseUrl = getBaseUrl(origin);
  const lastmod = payload.extracted_at ? payload.extracted_at.slice(0, 10) : undefined;
